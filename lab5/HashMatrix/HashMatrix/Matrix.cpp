...
TElem Matrix::modify(int i, int j, TElem e) {
	...
	if (e == NULL_TELEM)  //delete
	{
		...
		{                                              
			if ((table.elements[pos].line == i) && (table.elements[pos].column == j) && (table.elements[pos].value != DELETED_TELEM)) //found the element, it is not deleted
			{
			...
			{
				return NULL_TELEM; //found the element, it is deleted
			}
			step++;
			pos = hash(step, i, j, table.capacity);   //search the probe sequence
		}
...
void Matrix::resize()
{
	Triplet* new_elements = new Triplet[table.capacity * 2];
	...
	for (int i = 0; i < table.capacity; i++)
	{
		...
		if ((el.value != NULL_TELEM) && (el.value != DELETED_TELEM))
		{
			...
			while (new_elements[pos].value != NULL_TELEM) //while occupied
			{
				step++;
